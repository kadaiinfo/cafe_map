...
export default function MapView() {
    ...
    // 吹き出し表示を管理する関数
    const updatePopups = useCallback((selectedCafeId: string | null) => {
        markerElementsRef.current.forEach((element, cafeId) => {
            // 既存の吹き出しを削除
            const existingPopup = element.querySelector('.cafe-marker__popup')
            if (existingPopup) {
            ...
            if (cafeId === selectedCafeId) {
                ...
                popup.className = 'cafe-marker__popup'
                popup.textContent = 'ココ！'
    ...
    // 選択されたカフェが変更された時に吹き出しを更新
    useEffect(() => {
        updatePopups(selected?.id || null)
    }, [selected, updatePopups])
